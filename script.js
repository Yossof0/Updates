...
window.addEventListener('DOMContentLoaded', () => {
      ...
      if (useRealtimeDB && groupsRef) {
        groupsRef.orderByChild('ownerId').equalTo(adminId).once('value').then(snap => {
          ...
          Object.keys(data).forEach(k => {
            ...
            const title = document.createElement('div'); title.className = 'group-title'; title.textContent = g.name || k;
            const code = document.createElement('div'); code.className = 'group-code'; code.textContent = k;
            info.appendChild(title); info.appendChild(code);
            const actions = document.createElement('div'); actions.className = 'group-actions';
            const openBtn = document.createElement('button'); openBtn.textContent = 'Open'; openBtn.onclick = () => enterGroup(k);
                    const del = document.createElement('button'); del.textContent = 'Delete'; del.onclick = () => {
                      // remove without blocking confirm; show status in infoBox
                      groupsRef.child(k).remove().then(()=>{
                        try { realtimeDB.ref('groupMessages/'+k).remove(); } catch(e){}
                        renderGroups();
                        if (activeGroup===k){
                          activeGroup=null;
                          if(activeGroupNameEl) activeGroupNameEl.textContent='Select a group';
                        ...
                        if (infoBox) infoBox.textContent = 'Group deleted: ' + k;
  ...
  Object.keys(local).forEach(k => { const g = local[k]; if (g.ownerId === adminId) { const wrap=document.createElement('div'); wrap.className='group-item'; const info=document.createElement('div'); info.className='group-info'; const title=document.createElement('div'); title.className='group-title'; title.textContent=g.name||k; const code=document.createElement('div'); code.className='group-code'; code.textContent=k; info.appendChild(title); info.appendChild(code); const actions=document.createElement('div'); actions.className='group-actions'; const openBtn=document.createElement('button'); openBtn.textContent='Open'; openBtn.onclick=()=>enterGroup(k); const del=document.createElement('button'); del.textContent='Delete'; del.onclick=()=>{ /* no confirm: delete and notify */ delete local[k]; localStorage.setItem('localGroups', JSON.stringify(local)); renderGroups(); if (infoBox) infoBox.textContent = 'Group deleted: ' + k; }; actions.appendChild(openBtn); actions.appendChild(del); wrap.appendChild(info); wrap.appendChild(actions); groupsListEl.appendChild(wrap); } });
      }
    } else {
  ...
  if (joinGroupBtn) joinGroupBtn.addEventListener('click', async ()=>{
    ...
    if (useRealtimeDB && groupsRef){ groupsRef.child(gvc).once('value').then(snap=>{ if (!snap.exists()){ if (errorBox) errorBox.textContent='Group not found'; return; } if (!joinedGroups.includes(gvc)){ joinedGroups.push(gvc); localStorage.setItem('joinedGroups', JSON.stringify(joinedGroups)); } renderGroups(); }).catch(()=>{ if (errorBox) errorBox.textContent='Lookup failed'; }); }
    else { const local = JSON.parse(localStorage.getItem('localGroups')||'{}'); if (!local[gvc]){ if (errorBox) errorBox.textContent='Group not found (local)'; return; } if (!joinedGroups.includes(gvc)){ joinedGroups.push(gvc); localStorage.setItem('joinedGroups', JSON.stringify(joinedGroups)); } renderGroups(); }
  });
