...
window.addEventListener('DOMContentLoaded', () => {
  // Role is stored in localStorage; leave undefined if not set so we ask the visitor
  ...
  function setupRole(r) {
    ...
    if (composerBox) composerBox.style.display = (role === 'admin') ? 'flex' : 'none';
    renderMessages();
  }
  ...
  if (sendBtn) {
    sendBtn.addEventListener('click', () => {
      ...
      const msg = { text, role: effectiveRole, time: Date.now() };
      if (useRealtimeDB && messagesRef) {
        // push a new key then set the value to ensure we can reference the key for deletes
        const newRef = messagesRef.push();
        newRef.set(msg).then(() => {
          if (infoBox) infoBox.textContent = '✔ Message sent';
          if (textEl) textEl.value = '';
        }).catch((err) => { if (errorBox) errorBox.textContent = 'Send failed: ' + (err && err.message ? err.message : 'unknown'); });
      } else {
        messages.push(msg);
        localStorage.setItem('messages', JSON.stringify(messages));
        if (textEl) textEl.value = '';
  ...
  // When the page unloads, clean up realtime DB listeners if present
  window.addEventListener('beforeunload', () => {
    if (useRealtimeDB && messagesRef && typeof messagesRef.off === 'function') messagesRef.off();
  });
});
