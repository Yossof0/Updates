...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // token to avoid async render races
  let _renderGroupsToken = 0;
  ...
  // Broadcast channel for same-origin tabs (fallback, optional)
  let bc = null;
  try { if (typeof BroadcastChannel !== 'undefined') bc = new BroadcastChannel('zednyilman'); } catch(e) { bc = null; }

  // Helper: remove group from local joined lists and UI for current client
  function removeGroupLocally(gvc) {
    // remove from joinedGroups
    joinedGroups = joinedGroups.filter(x => x !== gvc);
    localStorage.setItem('joinedGroups', JSON.stringify(joinedGroups));
    // remove from localGroups store if present
    const local = JSON.parse(localStorage.getItem('localGroups') || '{}');
    if (local[gvc]) { delete local[gvc]; localStorage.setItem('localGroups', JSON.stringify(local)); }
    // remove local messages
    try { localStorage.removeItem('group:' + gvc); } catch(e){}
  ...
  // Notify other clients that a group was deleted
  function notifyGroupDeleted(gvc) {
    const ts = Date.now();
    // Realtime DB notification
    if (useRealtimeDB && realtimeDB) {
      try { realtimeDB.ref('groupDeletes/' + gvc).set({ deletedBy: adminId || null, time: ts }).catch(()=>{}); } catch(e){}
    } else {
      // localStorage marker (other tabs will receive storage event)
      try {
        const del = JSON.parse(localStorage.getItem('deletedGroups') || '{}');
        del[gvc] = ts;
        localStorage.setItem('deletedGroups', JSON.stringify(del));
      } catch (e) {}
    }
    // BroadcastChannel for same-origin tabs
    try { if (bc) bc.postMessage({ type: 'groupDeleted', gvc, time: ts }); } catch(e){}
  }

  // Listen for remote deletion notifications (Realtime DB)
  if (useRealtimeDB && realtimeDB) {
    try {
      realtimeDB.ref('groupDeletes').on('child_added', snap => {
        const gvc = snap.key;
        if (!gvc) return;
        removeGroupLocally(gvc);
      });
    } catch (e) {}
  }

  // Listen for localStorage deletion markers (other tabs)
  window.addEventListener('storage', (e) => {
    if (!e) return;
    if (e.key === 'deletedGroups') {
      try {
        const del = JSON.parse(e.newValue || '{}');
        Object.keys(del).forEach(gvc => removeGroupLocally(gvc));
      } catch (err) {}
    }
    // keep joinedGroups in sync if other tabs changed it
    if (e.key === 'joinedGroups') {
      try { joinedGroups = JSON.parse(e.newValue || '[]') || []; } catch(e){}
      renderGroups();
    }
  });
  ...
  if (bc) {
    try { bc.onmessage = (m) => { if (!m || !m.data) return; if (m.data.type === 'groupDeleted' && m.data.gvc) removeGroupLocally(m.data.gvc); }; } catch(e){}
  }
  ...
  function renderGroups() {
    ...
    const myToken = ++_renderGroupsToken;
    // clear existing list immediately to avoid duplicates
    groupsListEl.innerHTML = '';
    // Collect all unique group GVCs: owned + joined, deduped
    let groupGVCs = new Set();
  // track which GVCs have been appended to the DOM to avoid duplicates
  let displayedGVCs = new Set();
    let ownedGroups = [];
    let joinedButNotOwned = [];

    function cleanupDuplicateGroupNodes() {
      try {
        const seen = new Set();
        Array.from(groupsListEl.children).forEach(ch => {
          const gvc = ch.getAttribute && ch.getAttribute('data-gvc');
          if (!gvc) return;
          if (seen.has(gvc)) ch.remove(); else seen.add(gvc);
        });
      } catch (e) { /* ignore */ }
    }
    if (role === 'admin') {
      if (useRealtimeDB && groupsRef) {
        groupsRef.orderByChild('ownerId').equalTo(adminId).once('value').then(snap => {
          // if another render was requested meanwhile, abort
          if (myToken !== _renderGroupsToken) return;
          const data = snap.val() || {};
          ...
          ownedGroups.forEach(g => {
            if (displayedGVCs.has(g.gvc)) return;
            displayedGVCs.add(g.gvc);
            ...
            const del = document.createElement('button'); del.textContent = 'Delete'; del.onclick = () => {
              groupsRef.child(g.gvc).remove().then(()=>{
                ...
                localStorage.setItem('joinedGroups', JSON.stringify(joinedGroups));
                // notify other clients and cleanup locally
                notifyGroupDeleted(g.gvc);
                renderGroups();
            ...
            // avoid duplicates by DOM marker
            if (!groupsListEl.querySelector('[data-gvc="' + g.gvc + '"]')) {
              wrap.setAttribute('data-gvc', g.gvc);
              groupsListEl.appendChild(wrap);
            }
          });
          // Render joined but not owned groups
          joinedButNotOwned.forEach(gvc => {
            if (displayedGVCs.has(gvc)) return;
            displayedGVCs.add(gvc);
            const wrap = document.createElement('div'); wrap.className='group-item';
            const info = document.createElement('div'); info.className = 'group-info';
            ...
            actions.appendChild(openBtn); actions.appendChild(leave); wrap.appendChild(info); wrap.appendChild(actions);
            if (!groupsListEl.querySelector('[data-gvc="' + gvc + '"]')) {
              wrap.setAttribute('data-gvc', gvc);
              groupsListEl.appendChild(wrap);
            }
            // populate name
            if (useRealtimeDB && groupsRef) {
              groupsRef.child(gvc).once('value').then(snap => { if (myToken !== _renderGroupsToken) return; const gg = snap.val(); if (gg && gg.name) title.textContent = gg.name; else title.textContent = 'Group ' + gvc; }).catch(()=>{ title.textContent = 'Group ' + gvc; });
        ...
        ownedGroups.forEach(g => {
          ...
          // also notify other tabs/clients when deleting locally
          const oldDelHandler = del.onclick;
          del.onclick = () => { oldDelHandler(); try { notifyGroupDeleted(g.gvc); } catch(e){} };
          actions.appendChild(openBtn); actions.appendChild(del); wrap.appendChild(info); wrap.appendChild(actions);
          if (!groupsListEl.querySelector('[data-gvc="' + g.gvc + '"]')) { wrap.setAttribute('data-gvc', g.gvc); groupsListEl.appendChild(wrap); }
        });
        joinedButNotOwned.forEach(gvc => {
          if (displayedGVCs.has(gvc)) return;
          displayedGVCs.add(gvc);
          ...
          actions.appendChild(openBtn); actions.appendChild(leave); wrap.appendChild(info); wrap.appendChild(actions);
          if (!groupsListEl.querySelector('[data-gvc="' + gvc + '"]')) { wrap.setAttribute('data-gvc', gvc); groupsListEl.appendChild(wrap); }
          if (local[gvc] && local[gvc].name) title.textContent = local[gvc].name; else title.textContent = 'Group ' + gvc;
        });
      }
      // schedule a cleanup in case async callbacks created duplicates
      setTimeout(()=>{ if (myToken !== _renderGroupsToken) return; cleanupDuplicateGroupNodes(); }, 80);
    } else {
      ...
      joinedGroups.forEach(gvc => {
        ...
        if (displayedGVCs.has(gvc)) return; displayedGVCs.add(gvc);
        const wrap = document.createElement('div'); wrap.className='group-item';
        const info = document.createElement('div'); info.className='group-info';
        const title = document.createElement('div'); title.className='group-title'; title.textContent = 'Loading...';
        const code = document.createElement('div'); code.className='group-code'; code.textContent = gvc;
  ...
  // initial render via setupRole (avoids double render)
